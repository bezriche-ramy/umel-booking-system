    ...
    return (
        <section className="flux-journal" aria-labelledby="flux-title">
            <div className="flux-journal-head">
                <h2 id="flux-title" style={{ textWrap: "balance" }}>
                    Fragments de <em>collection</em>
                </h2>
            ...
            <div className="flux-journal-grid">
                {fluxImages.map((file, index) => (
                    <figure key={file}>
                        <Image
                            src={`/images/flux/${file}`}
                            alt={`Inspiration Umel Couture ${index + 1}`}
                            fill
                            sizes="(max-width: 700px) 48vw, (max-width: 1100px) 25vw, 18vw"
                        />
